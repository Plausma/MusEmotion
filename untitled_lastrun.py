...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    for thisMusicRatingLoop in musicRatingLoop:
        ...
        while continueRoutine and routineTimer.getTime() < 4.0:
            ...
            if prog.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > prog.tStartRefresh + 4-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        RepCount = RepCount + 1  # Set Routine start values for RepCount
        SmallBreakT.setText('rating saved, ' + str(musicRatingLoop.thisN + 1) + ' from 4 samples done\nGo take a break lol\npress [space] to listen to the next piece' if (musicRatingLoop.thisN + 1) % 2 == 0 and (musicRatingLoop.thisN + 1) < musicRatingLoop.nTotal else ('rating saved, ' + str(musicRatingLoop.thisN + 1) + ' from 4 samples done\npress [space] to listen to the next piece') if musicRatingLoop.thisN + 1 < musicRatingLoop.nTotal else ("Completed all " + str(musicRatingLoop.nTotal) + " samples\nthank you for your sacrifice\npress [space] to exit"))
        continuepress.keys = []
        continuepress.rt = []
        ...
        thisExp.addData('RepCount.routineEndVal', RepCount)  # Save end Routine value
